...
const useResponses = () => {
  ...
  const fetchResponses = async () => {
    ...
    setError(null);
    try {
      const response = await fetch(`${process.env.VERCEL_URL}/api/responses`);
      if (!response.ok) {
  ...
  const fetchSingleResponse = async (id: string) => {
    ...
    setError(null);
    try {
      const data = await fetch(`${process.env.VERCEL_URL}/api/responses?id=${id}`);
      if (!data.ok) {
  ...
  const createResponse = async (response: NewResponseForm) => {
    ...
    const data = await fetch(`${process.env.VERCEL_URL}/api/responses`, {
      method: "POST",
      headers: {
  ...
  const getUserById = async (id: number | string | undefined) => {
    try {
      const response = await fetch(`${process.env.VERCEL_URL}/api/users?id=${id}`);
      if (!response.ok) {
        throw new Error(`Error: ${response.status} ${response.statusText}`);
      }
